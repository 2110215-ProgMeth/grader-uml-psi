...
public class EdgeCaseTest {
	...
	void testComplexGenericWildcards() throws Exception {
		String javaCode = "import java.util.*; " + "public class ComplexGenerics<T extends Number & Comparable<T>> { "
	...
	void testSelfReferencingType() throws Exception {
		String javaCode = "public class SelfRef<T extends SelfRef<T>> { " + "private T self; "
				+ "public T getSelf() { return self; } " + "public void setSelf(T self) { this.self = self; } " + "}";
	...
	void testMultipleTypesInSameFile() throws Exception {
		String javaCode = "class First {} " + "interface Second {} " + "enum Third { A, B } " + "@interface Fourth {} "
				+ "record Fifth(String data) {} " + "public class Main {}";
	...
	void testDeeplyNestedGenerics() throws Exception {
		String javaCode = "import java.util.*; " + "public class DeepNested { "
	...
	void testMethodWithManyParameters() throws Exception {
		...
		String javaCode = "public class ManyParams { " + "public void method(" + params + ") {} " + "}";
	...
	void testMethodWithManyExceptions() throws Exception {
		String javaCode = "import java.io.*; " + "public class ManyExceptions { "
	...
	void testComplexAnnotationValues() throws Exception {
		String javaCode = "@interface ComplexAnno { " + "String[] values() default {\"a\", \"b\", \"c\"}; "
				+ "Class<?>[] types() default {String.class, Integer.class}; "
				+ "int[][] matrix() default {{1, 2}, {3, 4}}; " + "} "
				+ "@ComplexAnno(values = {\"x\", \"y\"}, types = {Object.class}) " + "public class AnnotatedComplex {}";
	...
	void testCircularGenericConstraints() throws Exception {
		String javaCode = "class A<T extends B<T>> {} " + "class B<U extends A<U>> {} "
	...
	void testMultiDimensionalArrays() throws Exception {
		String javaCode = "public class MultiArrays { " + "private int[][][] cube; "
	...
	void testExtremelyLongMethodName() throws Exception {
		...
		String javaCode = "public class LongMethod { " + "public void " + longMethodName + "() {} " + "}";
	...
	void testSpecialCharactersInStringLiterals() throws Exception {
		String javaCode = "public class SpecialChars { " + "private String special = \"\\n\\t\\r\\\\\\\"\"; "
	...
	void testMixedStaticAndNonStaticWithSameName() throws Exception {
		String javaCode = "public class MixedStatic { " + "public static void process() {} "
				+ "public void process(String input) {} " + "public static class Inner {} " + "public class Inner2 {} "
				+ "}";
	...
	void testSealedClassHierarchy() throws Exception {
		String javaCode = "public sealed class Shape permits Circle, Rectangle, Triangle {"
				+ "    protected abstract double area();" + "}" + "final class Circle extends Shape {"
				+ "    private double radius;" + "    protected double area() { return Math.PI * radius * radius; }"
				+ "}" + "non-sealed class Rectangle extends Shape {" + "    protected double area() { return 0; }" + "}"
				+ "sealed class Triangle extends Shape permits EquilateralTriangle {"
				+ "    protected double area() { return 0; }" + "}"
				+ "final class EquilateralTriangle extends Triangle {" + "    protected double area() { return 0; }"
				+ "}";
	...
	void testAnonymousClassInField() throws Exception {
		String javaCode = "public class Test {" + "    private Runnable task = new Runnable() {"
				+ "        private String name = \"anonymous\";" + "        public void run() {"
				+ "            System.out.println(name);" + "        }" + "        private void helper() {}" + "    };"
	...
	void testLocalClassInMethod() throws Exception {
		String javaCode = "public class OuterClass {" + "    public void outerMethod() {" + "        class LocalClass {"
				+ "            private int localField;" + "            public void localMethod() {}"
				+ "            class NestedLocal {" + "                void deepMethod() {}" + "            }"
				+ "        }" + "        LocalClass local = new LocalClass();" + "    }"
				+ "    private class RegularInner {" + "        void regularMethod() {}" + "    }" + "}";
	...
	void testTextBlocksInFields() throws Exception {
		String javaCode = "public class TextBlockTest {" + "    private String query = \"\"\""
				+ "        SELECT id, name" + "        FROM users" + "        WHERE active = true" + "        \"\"\";"
				+ "    private String json = \"\"\"" + "        {" + "          \"name\": \"John\","
				+ "          \"age\": 30" + "        }" + "        \"\"\";" + "    public String getFormattedText() {"
				+ "        return \"\"\"" + "            Hello %s," + "            Welcome to our service!"
				+ "            \"\"\".formatted(\"User\");" + "    }" + "}";
	...
	void testRecordWithComplexComponents() throws Exception {
		String javaCode = "import java.util.*;" + "public record ComplexRecord(" + "    String name,"
				+ "    List<Map<String, Integer>> data," + "    Object... extras"
				+ ") implements Comparable<ComplexRecord> {" + "    public ComplexRecord {"
				+ "        Objects.requireNonNull(name);" + "        if (data == null) data = List.of();" + "    }"
				+ "    public ComplexRecord(String name) {" + "        this(name, List.of());" + "    }"
				+ "    public int compareTo(ComplexRecord other) {" + "        return name.compareTo(other.name);"
				+ "    }" + "    public static ComplexRecord empty() {" + "        return new ComplexRecord(\"\");"
	...
	void testEnumWithComplexConstructors() throws Exception {
		String javaCode = "public enum HttpStatus {" + "    OK(200, \"OK\", true),"
				+ "    NOT_FOUND(404, \"Not Found\", false) {" + "        @Override"
				+ "        public String getCustomMessage() {" + "            return \"Resource not found\";"
				+ "        }" + "    }," + "    INTERNAL_ERROR(500, \"Internal Server Error\", false) {"
				+ "        @Override" + "        public String getCustomMessage() {"
				+ "            return \"Server error occurred\";" + "        }" + "        private void logError() {"
				+ "            System.err.println(\"Error logged\");" + "        }" + "    };" + "    "
				+ "    private final int code;" + "    private final String message;"
				+ "    private final boolean success;" + "    "
				+ "    HttpStatus(int code, String message, boolean success) {" + "        this.code = code;"
				...
				+ "    public static HttpStatus fromCode(int code) {" + "        for (HttpStatus status : values()) {"
				+ "            if (status.code == code) return status;" + "        }" + "        return null;" + "    }"
				+ "}";
	...
	void testInterfaceWithDefaultAndStaticMethods() throws Exception {
		String javaCode = "public interface AdvancedInterface<T extends Comparable<T>> {" + "    T process(T input);"
				+ "    " + "    default T processWithDefault(T input) {"
				+ "        System.out.println(\"Processing: \" + input);" + "        return process(input);" + "    }"
				+ "    " + "    default void log(String message) {"
				+ "        System.out.println(getClass().getSimpleName() + \": \" + message);" + "    }" + "    "
				+ "    static <U> void staticHelper(U item) {"
				+ "        System.out.println(\"Static helper: \" + item);" + "    }" + "    "
				+ "    static String getVersion() {" + "        return \"1.0\";" + "    }" + "    "
				...
				+ "    class NestedClass implements NestedInterface {" + "        public void nestedMethod() {}"
				+ "    }" + "}";
	...
	void testAnnotationWithComplexDefaultValues() throws Exception {
		String javaCode = "import java.lang.annotation.*;"
				...
				+ "@Retention(RetentionPolicy.RUNTIME)" + "@Repeatable(ConfigList.class)" + "public @interface Config {"
				+ "    String name() default \"\";" + "    String[] values() default {};"
				+ "    int priority() default 0;" + "    Class<?>[] types() default {Object.class};"
				+ "    ElementType target() default ElementType.TYPE;" + "    boolean enabled() default true;"
				+ "    long timeout() default 5000L;" + "    double ratio() default 1.0;"
				+ "    char separator() default ',';" + "    NestedEnum level() default NestedEnum.INFO;" + "    "
				+ "    enum NestedEnum {" + "        DEBUG, INFO, WARN, ERROR;" + "        private String description;"
				+ "        public String getDescription() { return description; }" + "    }" + "    "
				+ "    @interface NestedAnnotation {" + "        String value() default \"nested\";" + "    }" + "}"
				+ "@Target(ElementType.TYPE)" + "@Retention(RetentionPolicy.RUNTIME)" + "@interface ConfigList {"
				+ "    Config[] value();" + "}";
	...
	void testComplexGenericBounds() throws Exception {
		String javaCode = "import java.io.Serializable;" + "import java.util.*;" + "public class GenericBounds<"
				+ "    T extends Number & Comparable<T> & Serializable," + "    U extends List<? super T>,"
				+ "    V extends Map<String, ? extends Collection<T>>," + "    W extends GenericBounds<T, U, V, W>"
				+ "> implements Comparable<GenericBounds<T, U, V, W>> {" + "    " + "    private T numericValue;"
				+ "    private U listValue;" + "    private V mapValue;" + "    private W selfReference;" + "    "
				+ "    public <X extends T, Y extends Collection<? super X>> "
				+ "    void complexMethod(X item, Y collection, Map<? extends String, ? super Y> map) {}" + "    "
				+ "    public <A, B extends A, C extends List<B>> " + "    C chainedGenerics(A a, B b) { return null; }"
				+ "    " + "    public int compareTo(GenericBounds<T, U, V, W> other) { return 0; }" + "    "
				+ "    public static class Builder<" + "        T extends Number & Comparable<T> & Serializable,"
				+ "        U extends List<? super T>," + "        V extends Map<String, ? extends Collection<T>>,"
	...
	void testVarArgsWithGenerics() throws Exception {
		String javaCode = "import java.util.*;" + "public class VarArgsTest {" + "    @SafeVarargs"
				+ "    public final <T> void safeVarArgs(T... items) {}" + "    " + "    @SafeVarargs"
				+ "    public static <T> List<T> listOf(T... elements) {" + "        return Arrays.asList(elements);"
				+ "    }" + "    " + "    public <T extends Number> void numbersOnly(T... numbers) {}" + "    "
				+ "    public void mixedParams(String prefix, int count, Object... extras) {}" + "    "
				+ "    public <T> void genericArrays(T[] array, T... varargs) {}" + "    " + "    @SafeVarargs"
		...
		for (JsonNode method : methods) {
			...
			if (params.size() > 0) {
				...
				if (method.get("name").asText().contains("VarArgs") || method.get("name").asText().equals("mixedParams")
	...
	void testSwitchExpressions() throws Exception {
		String javaCode = "public class SwitchTest {" + "    public String processDay(int day) {"
				+ "        return switch (day) {" + "            case 1, 2, 3, 4, 5 -> \"Weekday\";"
				+ "            case 6, 7 -> \"Weekend\";" + "            default -> \"Invalid day\";" + "        };"
				+ "    }" + "    " + "    public int calculate(String operation, int a, int b) {"
				+ "        return switch (operation) {" + "            case \"add\" -> a + b;"
				+ "            case \"subtract\" -> a - b;" + "            case \"multiply\" -> {"
				+ "                System.out.println(\"Multiplying\");" + "                yield a * b;"
	...
	void testTypeInferenceWithVar() throws Exception {
		String javaCode = "import java.util.*;" + "public class VarTest {" + "    public void demonstrateVar() {"
				+ "        var list = new ArrayList<String>();" + "        var map = Map.of(\"key\", \"value\");"
				+ "        var stream = list.stream();" + "        " + "        for (var item : list) {"
				+ "            System.out.println(item);" + "        }" + "        "
				+ "        try (var scanner = new Scanner(System.in)) {" + "            var input = scanner.nextLine();"
				+ "        }" + "    }" + "    " + "    private Map<String, Integer> processData() {"
				+ "        var result = new HashMap<String, Integer>();" + "        return result;" + "    }" + "}";
